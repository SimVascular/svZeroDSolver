...
int main(int argc, char *argv[]) {
  ...
  for (auto block : model->blocks)
  {
    ...
    {
        model->nodes.push_back(
              new MODEL::Node("inlet:" + block->name));
    ...
    {
        model->nodes.push_back(
              new MODEL::Node(block->name + ":outlet"));
  ...
  Eigen::SparseMatrix<T> X = Eigen::SparseMatrix<T>(num_observations*model->dofhandler.size(), param_counter);
  Eigen::Matrix<T, Eigen::Dynamic, 1> Y = Eigen::Matrix<T, Eigen::Dynamic, 1>::Zero(num_observations*model->dofhandler.size());

  int num_dofs = model->dofhandler.size();
  for (size_t i = 0; i < num_observations; i++)
  {
    DEBUG_MSG("Assembling observation " << i);
    Eigen::Matrix<T, Eigen::Dynamic, 1> y = Eigen::Matrix<T, Eigen::Dynamic, 1>::Zero(model->dofhandler.size());
    Eigen::Matrix<T, Eigen::Dynamic, 1> dy = Eigen::Matrix<T, Eigen::Dynamic, 1>::Zero(model->dofhandler.size());
