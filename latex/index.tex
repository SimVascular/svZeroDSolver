...
Finally the \mbox{\hyperlink{namespace_i_o}{IO}} namespace provides everything related to reading and writing files. The \mbox{\hyperlink{class_i_o_1_1_config_reader}{IO\+::\+Config\+Reader}} reads the simultion config and IO\+::write\+\_\+csv and IO\+::write\+\_\+json are two different methods for writing the result files.
...
\DoxyCodeLine{brew install pybind11    \# Python bindings}
...
\DoxyCodeLine{sudo apt install libjsoncpp-\/dev      \# Standard json library}
\DoxyCodeLine{sudo apt install libsimdjson-\/dev     \# Fast json input parser}
\DoxyCodeLine{sudo apt install pybind11-\/dev        \# Python bindings}
...
\end{DoxyCode}
\hypertarget{index_autotoc_md10}{}\doxysubsection{Build on Sherlock}\label{index_autotoc_md10}

\begin{DoxyCode}{0}
\DoxyCodeLine{ml load cmake/3.23.1}
\DoxyCodeLine{ml load gcc/12.1.0}
\DoxyCodeLine{ml load python/3.9.0}
\DoxyCodeLine{ml load binutils/2.38}
\DoxyCodeLine{mkdir externals}
\DoxyCodeLine{cd externals}
\DoxyCodeLine{git clone https:\textcolor{comment}{//gitlab.com/libeigen/eigen.git}}
\DoxyCodeLine{git clone https:\textcolor{comment}{//github.com/simdjson/simdjson.git}}
\DoxyCodeLine{git clone https:\textcolor{comment}{//github.com/open-\/source-\/parsers/jsoncpp.git}}
\DoxyCodeLine{git clone https:\textcolor{comment}{//github.com/pybind/pybind11.git}}
\DoxyCodeLine{cd ..}
\DoxyCodeLine{mkdir Release}
\DoxyCodeLine{cd Release}
\DoxyCodeLine{cmake -\/DCMAKE\_BUILD\_TYPE=Release -\/DCLUSTER=ON -\/DCMAKE\_CXX\_COMPILER=/share/software/user/open/gcc/12.1.0/bin/g++ -\/DCMAKE\_C\_COMPILER=/share/software/user/open/gcc/12.1.0/bin/gcc -\/DPYTHON\_EXECUTABLE=/share/software/user/open/python/3.9.0/bin/python3 ..}
\DoxyCodeLine{cmake -\/-\/build }

\end{DoxyCode}
\hypertarget{index_autotoc_md11}{}\doxysection{Run sv\+Zero\+DSolver}\label{index_autotoc_md11}
...
{\ttfamily path/to/config.\+json} and {\ttfamily path/to/output} should be replaced by the correct paths to the input and output file, respectively.\hypertarget{index_autotoc_md12}{}\doxysubsection{Simulation parameters}\label{index_autotoc_md12}
...
\end{longtabu}
\hypertarget{index_autotoc_md13}{}\doxysection{Developer Guide}\label{index_autotoc_md13}
