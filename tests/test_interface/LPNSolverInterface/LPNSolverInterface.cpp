...
void LPNSolverInterface::load_library(const std::string& interface_lib) {
  ...
  if (!library_handle_) {
    std::cerr << "Error loading shared library '" << interface_lib
  ...
  *(void**)(&lpn_initialize_) = dlsym(library_handle_, "initialize");
  if (!lpn_initialize_) {
    std::cerr << "Error loading function 'initialize' with error: " << dlerror()
              << std::endl;
  ...
  *(void**)(&lpn_increment_time_) = dlsym(library_handle_, "increment_time");
  if (!lpn_increment_time_) {
    std::cerr << "Error loading function 'increment_time' with error: "
              << dlerror() << std::endl;
    dlclose(library_handle_);
    return;
  }
  ...
  *(void**)(&lpn_run_simulation_) = dlsym(library_handle_, "run_simulation");
  if (!lpn_run_simulation_) {
    std::cerr << "Error loading function 'run_simulation' with error: "
              << dlerror() << std::endl;
    dlclose(library_handle_);
    return;
  }
  ...
  *(void**)(&lpn_update_block_params_) =
      dlsym(library_handle_, "update_block_params");
  if (!lpn_update_block_params_) {
    std::cerr << "Error loading function 'update_block_params' with error: "
              << dlerror() << std::endl;
    dlclose(library_handle_);
    return;
  }
  ...
  *(void**)(&lpn_read_block_params_) =
  ...
  if (!lpn_read_block_params_) {
    std::cerr << "Error loading function 'read_block_params' with error: "
              << dlerror() << std::endl;
    dlclose(library_handle_);
    return;
  }
  ...
  *(void**)(&lpn_get_block_node_IDs_) =
      dlsym(library_handle_, "get_block_node_IDs");
  if (!lpn_get_block_node_IDs_) {
    std::cerr << "Error loading function 'lpn_get_block_node_IDs' with error: "
              << dlerror() << std::endl;
    dlclose(library_handle_);
    return;
  }
  ...
  *(void**)(&lpn_update_state_) = dlsym(library_handle_, "update_state");
  if (!lpn_update_state_) {
    std::cerr << "Error loading function 'lpn_update_state' with error: "
              << dlerror() << std::endl;
    dlclose(library_handle_);
    return;
  }
  ...
  *(void**)(&lpn_return_y_) = dlsym(library_handle_, "return_y");
  if (!lpn_return_y_) {
    std::cerr << "Error loading function 'lpn_return_y' with error: "
              << dlerror() << std::endl;
    dlclose(library_handle_);
    return;
  }
  ...
  *(void**)(&lpn_return_ydot_) = dlsym(library_handle_, "return_ydot");
  if (!lpn_return_ydot_) {
    std::cerr << "Error loading function 'lpn_return_ydot' with error: "
              << dlerror() << std::endl;
    dlclose(library_handle_);
    return;
  }
  ...
  *(void**)(&lpn_set_external_step_size_) =
      dlsym(library_handle_, "set_external_step_size");
...
//
void LPNSolverInterface::run_simulation(const double time,
                                        std::vector<double>& output_times,
                                        std::vector<double>& output_solutions,
                                        int& error_code) {
  lpn_run_simulation_(problem_id_, time, output_times, output_solutions,
                      error_code);
}
...
void LPNSolverInterface::update_state(std::vector<double> state_y,
