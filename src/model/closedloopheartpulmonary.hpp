...
namespace MODEL
{
    ...
    public:
        ...
        /**
         * @brief Update the solution-dependent contributions of the element in a sparse system
         *
         * @param system System to update contributions at
       ...
       std::map<std::string, int> get_num_triplets();
        ...
        /**
         * @brief Compute sub-expressions that are part of atrial elastance and depends on atrial volume from the solution vector
         *
         * @param y Current solution
         */
    ...
    private:
        Parameters params;
        // Below variables change every timestep and are then combined with expressions that are updated with solution 
        T AA;  // Atrial activation function
        T Elv; // LV elastance
        T Erv; // RV elastance
        T psi_ra, psi_la, psi_ra_derivative, psi_la_derivative; // Expressions for atrial activation
        T valves[16]; 
    };
    ...
    template <typename T>
    void ClosedLoopHeartPulmonary<T>::update_constant(ALGEBRA::SparseSystem<T> &system)
    {
        // DOF 2, Eq 1: Aortic pressure
        system.E.coeffRef(this->global_eqn_ids[1], this->global_var_ids[2]) = params.Cpa;
        ...
        system.E.coeffRef(this->global_eqn_ids[3], this->global_var_ids[5]) = params.Lra_v;
        ...
        system.E.coeffRef(this->global_eqn_ids[6], this->global_var_ids[8]) = params.Lrv_a;
        // DOF 9, Eq 7: Pulmonary pressure
        system.E.coeffRef(this->global_eqn_ids[7], this->global_var_ids[9]) = params.Cp;
        ...
        system.E.coeffRef(this->global_eqn_ids[10], this->global_var_ids[12]) = params.Lla_v;
        ...
        system.E.coeffRef(this->global_eqn_ids[13], this->global_var_ids[15]) = params.Llv_a;
    }

    template <typename T>
    void ClosedLoopHeartPulmonary<T>::update_time(ALGEBRA::SparseSystem<T> &system, T time)
    {
        this->get_activation_and_elastance_functions(time);
    }

    template <typename T>
    void ClosedLoopHeartPulmonary<T>::update_solution(ALGEBRA::SparseSystem<T> &system, Eigen::Matrix<T, Eigen::Dynamic, 1> &y)
    {
        ...
        system.F.coeffRef(this->global_eqn_ids[0], this->global_var_ids[4]) = -this->AA*params.Emax_ra;
        system.C(this->global_eqn_ids[0]) = this->AA*params.Emax_ra*params.Vaso_ra + psi_ra*(this->AA-1.0);
        system.D.coeffRef(this->global_eqn_ids[0], this->global_var_ids[4]) = psi_ra_derivative*(this->AA-1.0);

        // DOF 1: Flow into right atrium (no equation)

        // DOF 2, Eq 1: Aortic pressure
        system.F.coeffRef(this->global_eqn_ids[1], this->global_var_ids[15]) = -valves[15];
        system.F.coeffRef(this->global_eqn_ids[1], this->global_var_ids[3]) = 1.0;

        // DOF 3: Flow into aorta (no equation)

        // DOF 4, Eq 2: Right atrium volume
        system.F.coeffRef(this->global_eqn_ids[2], this->global_var_ids[5]) = 1.0*valves[5];
        system.F.coeffRef(this->global_eqn_ids[2], this->global_var_ids[1]) = -1.0;

        // DOF 5, Eq 3: Right atrium outflow
        system.F.coeffRef(this->global_eqn_ids[3], this->global_var_ids[5]) = params.Rra_v*valves[5];
        ...
        system.F.coeffRef(this->global_eqn_ids[4], this->global_var_ids[7]) = -this->Erv;
        system.C(this->global_eqn_ids[4]) = this->Erv*params.Vrv_u;

        // DOF 7, Eq 5: Right ventricle volume
        system.F.coeffRef(this->global_eqn_ids[5], this->global_var_ids[5]) = -1.0*valves[5];
        system.F.coeffRef(this->global_eqn_ids[5], this->global_var_ids[8]) = 1.0*valves[8];
        ...
        system.F.coeffRef(this->global_eqn_ids[6], this->global_var_ids[8]) = params.Rrv_a*valves[8];

        // DOF 9, Eq 7: Pulmonary pressure
        system.F.coeffRef(this->global_eqn_ids[7], this->global_var_ids[8]) = -valves[8];
        system.F.coeffRef(this->global_eqn_ids[7], this->global_var_ids[9]) = 1.0/params.Rpd;
        system.F.coeffRef(this->global_eqn_ids[7], this->global_var_ids[10]) = -1.0/params.Rpd;

        // DOF 10, Eq 8: Left atrium pressure
        system.F.coeffRef(this->global_eqn_ids[8], this->global_var_ids[10]) = 1.0;
        system.F.coeffRef(this->global_eqn_ids[8], this->global_var_ids[11]) = -this->AA*params.Emax_la;
        system.C(this->global_eqn_ids[8]) = this->AA*params.Emax_la*params.Vaso_la + psi_la*(this->AA-1.0);
        system.D.coeffRef(this->global_eqn_ids[8], this->global_var_ids[11]) = psi_la_derivative*(this->AA-1.0);

        // DOF 11, Eq 9: Left atrium volume
        system.F.coeffRef(this->global_eqn_ids[9], this->global_var_ids[8]) = -1.0*valves[8];
        system.F.coeffRef(this->global_eqn_ids[9], this->global_var_ids[12]) = 1.0*valves[12];
        ...
        system.F.coeffRef(this->global_eqn_ids[10], this->global_var_ids[12]) = params.Rla_v*valves[12];

        // DOF 13, Eq 11: Left ventricle pressure
        system.F.coeffRef(this->global_eqn_ids[11], this->global_var_ids[13]) = 1.0;
        system.F.coeffRef(this->global_eqn_ids[11], this->global_var_ids[14]) = -this->Elv;
        system.C(this->global_eqn_ids[11]) = this->Elv*params.Vlv_u;

        // DOF 14, Eq 12: Left ventricle volume
        system.F.coeffRef(this->global_eqn_ids[12], this->global_var_ids[12]) = -1.0*valves[12];
        system.F.coeffRef(this->global_eqn_ids[12], this->global_var_ids[15]) = 1.0*valves[15];
    ...
    template <typename T>
    void ClosedLoopHeartPulmonary<T>::get_activation_and_elastance_functions(T time)
    {
        ...
        else if ((t_in_cycle >= (T_cardiac-Tsa)+tpwave) and (t_in_cycle < T_cardiac))
        {
            AA = (0.5)*(1.0-cos(2.0*PI*(t_in_cycle-tpwave-(T_cardiac-Tsa))/Tsa));
        }
        else
        {
            AA = 0.0;
...
//                    { 0.000004903 ,  0.000005805}}, sizeof(Ft_elastance));
        ...
        for(auto i=0; i < num_elast_modes; i++)
            Elv_i = Elv_i + (Ft_elastance[i][0])*cos(2.0*PI*i*t_in_cycle/T_cardiac)
    ...
    template <typename T>
    void ClosedLoopHeartPulmonary<T>::get_psi_ra_la(Eigen::Matrix<T, Eigen::Dynamic, 1> &y)
    {
        auto RA_volume = y[this->global_var_ids[4]];
        auto LA_volume = y[this->global_var_ids[11]];
        psi_ra = params.Kxp_ra*(exp((RA_volume-params.Vaso_ra)*params.Kxv_ra)-1.0);
        psi_la = params.Kxp_la*(exp((LA_volume-params.Vaso_la)*params.Kxv_la)-1.0);

        psi_ra_derivative = params.Kxp_ra*exp((RA_volume-params.Vaso_ra)*params.Kxv_ra)*params.Kxv_ra;
        psi_la_derivative = params.Kxp_la*exp((LA_volume-params.Vaso_la)*params.Kxv_la)*params.Kxv_la;

    }

    template <typename T>
    void ClosedLoopHeartPulmonary<T>::get_valve_positions(Eigen::Matrix<T, Eigen::Dynamic, 1> &y)
    {
...
//  }
    ...
    template <typename T>
    void ClosedLoopHeartPulmonary<T>::get_parameter_value(std::string message, T &param_value) 
    {
        if (message == "iml")
        {
