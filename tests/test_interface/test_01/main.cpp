...
int main(int argc, char** argv) {
  ...
  // Interface blocks flow boundary conditions (neumann boundary conditions for
  // the 3D flow solver)
    ...
    if (block_name == "outlet_aorta") {
      if ((num_inlet_nodes != 1) || (num_outlet_nodes != 0)) {
        throw std::runtime_error(
            "Wrong number of inlets/outlets for outlet_aorta");
      }
    } else {
      if ((num_inlet_nodes != 0) || (num_outlet_nodes != 1)) {
        throw std::runtime_error("Wrong number of inlets/outlets for " +
                                 block_name);
      }
    }
  }
