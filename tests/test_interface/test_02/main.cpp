...
// This test mimics the coupling of svZeroDSolver with an external parameter
// estimation code (eg. Tulip) A coronary model is used and parameters of the
// BCs are updated and then compared to a reference solution
...
//---------------------------------------------------------------------------------------
// Compare mean flow/pressure in aorta and coronary with pre-computed
// ("correct") values
//---------------------------------------------------------------------------------------
std::string check_simulation_results(
    double mean_aortic_flow, double mean_aortic_pressure,
    double mean_bc_lca1_outlet_flow, double mean_bc_lca1_outlet_pressure,
    double correct_mean_aortic_flow, double correct_mean_aortic_pressure,
    double correct_mean_bc_lca1_outlet_flow,
    double correct_mean_bc_lca1_outlet_pressure, bool& is_wrong) {
  double error_limit = 0.01;
  std::vector<std::string> wrong_quantities;
  if (abs(mean_aortic_flow - correct_mean_aortic_flow) /
          correct_mean_aortic_flow >
      error_limit) {
    is_wrong = true;
    wrong_quantities.push_back("Mean aortic flow");
  }
  if (abs(mean_aortic_pressure - correct_mean_aortic_pressure) /
          correct_mean_aortic_pressure >
      error_limit) {
    is_wrong = true;
    wrong_quantities.push_back("Mean aortic pressure");
  }
  if (abs(mean_bc_lca1_outlet_flow - correct_mean_bc_lca1_outlet_flow) /
          correct_mean_bc_lca1_outlet_flow >
      error_limit) {
    is_wrong = true;
    wrong_quantities.push_back("Mean BC_lca1 outlet flow");
  }
  if (abs(mean_bc_lca1_outlet_pressure - correct_mean_bc_lca1_outlet_pressure) /
          correct_mean_bc_lca1_outlet_pressure >
      error_limit) {
    is_wrong = true;
...
int main(int argc, char** argv) {
  ...
  if ((num_inlet_nodes != 1) && (num_outlet_nodes != 1)) {
    throw std::runtime_error(
        "Wrong number of inlets/outlets for J_heart_outlet");
  }
  ...
  bool is_wrong = false;
  std::string error_msg;
  error_msg = check_simulation_results(mean_aortic_flow, mean_aortic_pressure,
                                       mean_bc_lca1_outlet_flow,
                                       mean_bc_lca1_outlet_pressure, 63.3137,
                                       101.139, 0.135942, 3.17525, is_wrong);
  if (is_wrong) {
    throw std::runtime_error(
        "After initial simulation, error in the following quantities: " +
        error_msg);
  }
  ...
  for (int i = 0; i < interface.block_names_.size(); i++) {
    ...
    if ((block_name.substr(0, 6) == "BC_lca") ||
      ...
      for (int j = 0; j < num_coronary_params; j++) {
        if (abs(coronary_params[j] - correct_params[j]) / correct_params[j] >
            0.01) {
          throw std::runtime_error("Wrong parameters read from block " +
                                   block_name);
        }
      }
      ...
      for (int j = 0; j < 3; j++) {
        ...
        if (abs(coronary_params[j] - correct_params[j]) / correct_params[j] >
            0.01) {
          throw std::runtime_error("Wrong parameters after update for block " +
                                   block_name);
        }
      }
    }
  ...
  error_msg = check_simulation_results(mean_aortic_flow, mean_aortic_pressure,
                                       mean_bc_lca1_outlet_flow,
                                       mean_bc_lca1_outlet_pressure, 63.2715,
                                       101.232, 0.0691725, 3.17279, is_wrong);
  if (is_wrong) {
    throw std::runtime_error(
        "After parameter update, error in the following quantities: " +
        error_msg);
  }

  // Test restarting simulation by overwriting state with prescribed initial
  // condition and restoring initial parameters Check if correct parameters are
  // read and then update block parameters
  for (int i = 0; i < interface.block_names_.size(); i++) {
    std::string block_name = interface.block_names_[i];
    if ((block_name.substr(0, 6) == "BC_lca") ||
        (block_name.substr(0, 6) == "BC_rca")) {
      ...
      interface.read_block_params(block_name, coronary_params);
      for (int j = 0; j < 3; j++) {
        if (abs(coronary_params[j] - initial_params[j]) / initial_params[j] >
            0.01) {
          throw std::runtime_error("Wrong parameters after update for block " +
                                   block_name);
        }
  ...
  error_msg = check_simulation_results(mean_aortic_flow, mean_aortic_pressure,
                                       mean_bc_lca1_outlet_flow,
                                       mean_bc_lca1_outlet_pressure, 63.329,
                                       101.158, 0.135971, 3.17321, is_wrong);
  if (is_wrong) {
    throw std::runtime_error(
        "After restart simulation, error in the following quantities: " +
        error_msg);
  }
