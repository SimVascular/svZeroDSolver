#ifndef SVZERODSOLVER_READER_H_
#define SVZERODSOLVER_READER_H_
...
private:
    ...
    static TimeDependentParameter<T> get_time_dependent_parameter(simdjson::dom::element &json_times, simdjson::dom::element &json_values);
};
...
template <typename T>
TimeDependentParameter<T> ConfigReader<T>::get_time_dependent_parameter(simdjson::dom::element &json_times, simdjson::dom::element &json_values)
{
    std::vector<T> times;
    std::vector<T> values;
...
template <typename T>
Model<T> ConfigReader<T>::get_model()
{
    // Create blog mapping
    Model<T> model;
