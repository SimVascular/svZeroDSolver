...
namespace MODEL
{
    ...
    template <typename T>
    PressureReferenceBC<T>::PressureReferenceBC(TimeDependentParameter<T> P, std::string name) : Block<T>(name)
    {
        this->name = name;
        this->params.P = P;
    }
    ...
    template <typename T>
    void PressureReferenceBC<T>::update_constant(ALGEBRA::DenseSystem<T> &system)
    {
        system.F(this->global_eqn_ids[0], this->global_var_ids[0]) = 1.0;
    }

    template <typename T>
    void PressureReferenceBC<T>::update_time(ALGEBRA::DenseSystem<T> &system, T time)
    {
    ...
    template <typename T>
    void PressureReferenceBC<T>::update_time(ALGEBRA::SparseSystem<T> &system, T time)
    {
        system.C(this->global_eqn_ids[0]) = -params.P.get(time);
    }
