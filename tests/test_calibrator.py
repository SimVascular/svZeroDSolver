...
def test_steady_flow_calibration_R_only():
    """Calibrate only R_poiseuille while holding C, L and stenosis_coefficient
    constant at their input-file values. The observations were generated by a
    ground-truth forward simulation with R=100, C=1e-4, L=1.0, S=0; the input
    file starts R at 50 and the other parameters at the ground truth, so the
    calibrator should recover R=100 and leave the others untouched."""
    testfile = os.path.join(
        this_file_dir, "cases", "steadyFlow_calibration_R_only.json"
    )
