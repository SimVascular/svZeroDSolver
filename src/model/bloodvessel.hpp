...
namespace MODEL
{
     ...
     * \f]
     *
     * with the stenosis resistance \f$ R_{ste}=K_{t} \frac{\rho}{2 A_{o}^{2}}\left(\frac{A_{o}}{A_{s}}-1\right)^{2}|Q_{in}^{e}| \f$.
     * The constant part of the equation is summarized in \ref Parameters::stenosis_coefficient. \f$R\f$,
     * \f$C\f$, and \f$L\f$ refer to Poisieuille resistance, capacitance and inductance, respectively.
     *
    ...
    template <typename T>
    void BloodVessel<T>::update_solution(ALGEBRA::DenseSystem<T> &system, Eigen::Matrix<T, Eigen::Dynamic, 1> &y)
    {
