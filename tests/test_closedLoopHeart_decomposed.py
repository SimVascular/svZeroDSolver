...
(ChamberElastanceInductor, ValveTanh, BloodVessel, BloodVesselRC) and
compares with the monolithic ClosedLoopHeartAndPulmonary block.

References:
  - Sankaran et al. 2012 (patient-specific multiscale modeling)
  - Menon et al. 2023 (predictors of myocardial ischemia)
"""
...
def test_decomposed_closedLoopHeart():
    ...
    fig.tight_layout()
    save_fig(fig, "decomp_01_aortic.png")
    ...
    for ax, (label, m_vol, d_vol) in zip(axes.flat, chamber_map):
        ...
        ax.legend(fontsize=9); ax.grid(True, alpha=0.3)
    fig.tight_layout()
    save_fig(fig, "decomp_02_pv_loops.png")
    ...
    for ax, (label, m_name, d_name) in zip(axes.flat, flow_map):
        ...
        ax.legend(fontsize=9); ax.grid(True, alpha=0.3)
    fig.tight_layout()
    save_fig(fig, "decomp_03_valve_flows.png")
    ...
    for ax, (label, m_name, d_name) in zip(axes.flat, flow_map):
        ...
        ax.legend(fontsize=9); ax.grid(True, alpha=0.3)
    fig.tight_layout()
    save_fig(fig, "decomp_04_valve_dQdt.png")
    ...
    for ax, (label, m_name, d_name) in zip(axes.flat, chamber_map):
        ...
        ax.legend(fontsize=9); ax.grid(True, alpha=0.3)
    fig.tight_layout()
    save_fig(fig, "decomp_05_volumes.png")
    ...
    all_pass = True
    for col, label in [("pressure_in", "Aortic pressure"),
        ...
        if status == "FAIL":
            all_pass = False
        print(f"  {label:20s}: mono={mm:8.2f}  decomp={dm:8.2f}"
              f"  rel_diff={rd:.4f}  [{status}]")

    mono_pp = np.ptp(mono_aorta["pressure_in"].values)
    decomp_pp = np.ptp(decomp_aorta["pressure_in"].values)
    print(f"  {'Pulse pressure':20s}: mono={mono_pp:8.2f}"
          f"  decomp={decomp_pp:8.2f}")
    print("=" * 80)
