/**
 * @file resistancewithdistalpressure.hpp
 * @brief MODEL::ResistanceWithDistalPressure source file
 */
...
namespace MODEL
{
    ...
    /**
     * @brief Resistance with distal pressure boundary condition element.
     *
     * \f[
     ...
     */
    template <typename T>
    class ResistanceWithDistalPressure : public Block<T>
    {
    public:
        /**
        ...
        /**
         * @brief Construct a new ResistanceWithDistalPressure object
         *
         * @param R Time-dependent resistance
         * @param Pd Time-dependent distal pressure
         * @param name Name
         */
        ResistanceWithDistalPressure(TimeDependentParameter<T> R, TimeDependentParameter<T> Pd, std::string name);

        /**
         * @brief Destroy the ResistanceWithDistalPressure object
         *
         */
    ...
    template <typename T>
    ResistanceWithDistalPressure<T>::ResistanceWithDistalPressure(TimeDependentParameter<T> R, TimeDependentParameter<T> Pd, std::string name) : Block<T>(name)
    {
        this->name = name;
        this->params.R = R;
        this->params.Pd = Pd;
    ...
    template <typename T>
    ResistanceWithDistalPressure<T>::~ResistanceWithDistalPressure()
    {
    }

    template <typename T>
    void ResistanceWithDistalPressure<T>::setup_dofs(DOFHandler &dofhandler)
    {
        Block<T>::setup_dofs_(dofhandler, 1, 0);
    }

    template <typename T>
    void ResistanceWithDistalPressure<T>::update_constant(ALGEBRA::DenseSystem<T> &system)
    {
        system.F(this->global_eqn_ids[0], this->global_var_ids[0]) = 1.0;
    }

    template <typename T>
    void ResistanceWithDistalPressure<T>::update_time(ALGEBRA::DenseSystem<T> &system, T time)
    {
    ...
    template <typename T>
    void ResistanceWithDistalPressure<T>::update_constant(ALGEBRA::SparseSystem<T> &system)
    {
        system.F.coeffRef(this->global_eqn_ids[0], this->global_var_ids[0]) = 1.0;
    }

    template <typename T>
    void ResistanceWithDistalPressure<T>::update_time(ALGEBRA::SparseSystem<T> &system, T time)
    {
    ...
    template <typename T>
    void ResistanceWithDistalPressure<T>::to_steady()
    {
        params.R.to_steady();
        params.Pd.to_steady();
    }
